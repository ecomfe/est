...
!function(){function t(){x=document.createElement("canvas"),g=x.getContext("2d"),x.style.position="absolute",x.style.top=0,x.style.left=0,x.style.zIndex=1,x.style.pointerEvents="none",x.width=I,x.height=T,document.body.appendChild(x)}function e(){x&&x.parentNode.removeChild(x)}function n(t){var e=getComputedStyle(t).color;if(!e)return[255,255,255];try{return e.match(/(\d+), (\d+), (\d+)/).slice(1)}catch(n){return[255,255,255]}}function r(t){var e=m.getCursor(),r=m.cursorCoords(),a=document.elementFromPoint(r.left-5,r.top+5);t=m.getTokenAt(e),t&&(t=t.type);for(var i=u(P.min,P.max),c=n(a),l=i;l--;)S[j]=o(r.left+10,r.top,c),j=(j+1)%O}function o(t,e,n){var r={x:t,y:e+10,alpha:1,color:n};return 1===v?(r.size=u(2,4),r.vx=F.x[0]+Math.random()*(F.x[1]-F.x[0]),r.vy=F.y[0]+Math.random()*(F.y[1]-F.y[0])):2===v&&(r.size=u(2,8),r.drag=.92,r.vx=u(-3,3),r.vy=u(-3,3),r.wander=.15,r.theta=u(0,360)*Math.PI/180),r}function a(t){t.vy+=A,t.x+=t.vx,t.y+=t.vy,t.alpha*=E,g.fillStyle="rgba("+t.color[0]+","+t.color[1]+","+t.color[2]+","+t.alpha+")",g.fillRect(Math.round(t.x-1),Math.round(t.y-1),t.size,t.size)}function i(t){t.x+=t.vx,t.y+=t.vy,t.vx*=t.drag,t.vy*=t.drag,t.theta+=u(-.5,.5),t.vx+=.1*Math.sin(t.theta),t.vy+=.1*Math.cos(t.theta),t.size*=.96,g.fillStyle="rgba("+t.color[0]+","+t.color[1]+","+t.color[2]+","+t.alpha+")",g.beginPath(),g.arc(Math.round(t.x-1),Math.round(t.y-1),t.size,0,2*Math.PI),g.fill()}function c(t){for(var e,n=S.length;n--;)e=S[n],!e||e.alpha<.01||e.size<=.5||(1===v?a(e):2===v&&i(e))}function l(t){M=C=t}function u(t,e){return e||(e=t,t=0),t+~~(Math.random()*(e-t+1))}function f(t,e){var n=!1;return function(){n||(t.apply(this,arguments),n=!0,setTimeout(function(){n=!1},e))}}function h(){g.clearRect(0,0,I,T);var t=(new Date).getTime();z||(last_time=t);var e=(t-z)/1e3;if(z=t,M>0){M-=e;var n=M/C*w,r=u(-n,n),o=u(-n,n);p.style.transform="translate("+r+"px,"+o+"px)"}c(),b=requestAnimationFrame(h)}function d(){cancelAnimationFrame(b)}function s(){k(.3),R()}function y(t){if(!t)return!0;if("[object Array]"===Object.prototype.toString.call(t))return!t.length;if("[object Object]"===Object.prototype.toString.call(t)){var e=!0;for(var n in t)if(t.hasOwnProperty(n)&&"toString"!==n){e=!1;break}return e}return!1}var v,p,m,g,x,b,M=0,C=0,w=5,z=0,S=[],j=0,O=500,P={min:5,max:10},A=.08,E=.96,F={x:[-1,1],y:[-3.5,-1.5]},I=window.innerWidth,T=window.innerHeight,k=f(l,100),R=f(r,100),W=0;CodeMirror.defineOption("blastCode",!1,function(n,r,o){r?(m=n,m.state.blastCode=!0,v=r.effect||2,p=m.getWrapperElement(),0===W&&(t(),h()),y(o)&&(W++,m.on("change",s))):y(o)||(n.off("change",s),W--,0===W&&(e(),d()))})}();
